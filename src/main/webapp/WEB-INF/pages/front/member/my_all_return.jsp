...
<div class="row content">
    ...
    <div class="lf mycontent">
        <div class="rfcon_tit">
            <li class="${orderActive=="all_return"?"lihover":""}"><a href="<%=basePath%>pages/front/member/findMyReturn?type=all_return">全部订单</a></li>
            <i class="lf">|</i>
            <li class="${orderActive=="apply_return"?"lihover":""}"><a href="<%=basePath%>pages/front/member/findMyReturn?type=apply_return">申请退货</a></li>
            <i class="lf">|</i>
            <li class="${orderActive=="agree_return"?"lihover":""}"><a href="<%=basePath%>pages/front/member/findMyReturn?type=agree_return">同意退货</a></li>
            <i class="lf">|</i>
            <li class="${orderActive=="do_not_agree_return"?"lihover":""}"><a href="<%=basePath%>pages/front/member/findMyReturn?type=do_not_agree_return">拒绝退货</a></li>
            <i class="lf">|</i>
            <li class="${orderActive=="send"?"lihover":""}"><a href="<%=basePath%>pages/front/member/findMyReturn?type=send">已发货</a></li>
            <i class="lf">|</i>
            <li class="${orderActive=="com"?"lihover":""}"><a href="<%=basePath%>pages/front/member/findMyReturn?type=com">已完成</a></li>
            <div class="clr"></div>
        </div>
        <div class="od_info">
            <table border="0" cellpadding="0" cellspacing="0" id="orderTable">
                <c:forEach items="${list}" var="item">
                    <tr class="IndentTitle">
                        ...
                        <th class="f3">
                            <div class="store">退款金额：${item.price}</div>
                        </th>
                        <th class="f4">
                            ...
                            <div class="store">
                                <c:if test="${item.doModelList!=null}">
                                    ...
                                    <c:forEach items="${item.doModelList}" var="doModel">
                                        <a href="<%=basePath%>pages/front/member/${doModel.url}&returnMainId=${item.returnMainId}">${doModel.name}</a>&nbsp;
                                    </c:forEach>
                                </c:if>
                            </div>
                        </th>
                    </tr>
                    <c:forEach items="${item.goodsList}" var="model">
                        <tr>
                            <td class="first-child">
                                <div class="t1">
                                    <div class="lf od-pic"><img src="<%=basePath%>/resources/upload/goodsPic/${model.pic}"></div>
                                    <div class="lf od-txt">
                                        <p>${model.goodsName}</p>
                                        <p class="color02">下单时间：${item.createTime}</p>
                                    </div>
                                    <div class="clr"></div>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="t2">x${model.count}</div>
                            </td>
                        </tr>
                    </c:forEach>
                </c:forEach>
                    ...
                    layui.use('laypage', function(){
                        ...
                        laypage.render({
                            ...
                            ,jump: function(obj, first){
                                ...
                                if(!first){
                                    $("#orderTable").empty();
                                    $.post("<%=basePath%>pages/front/member/findMyReturnForPage?type=${orderActive}",{pageNum:obj.curr},function (data) {
                                        ...
                                        for(var i=0;i<data.length;i++){
                                            ...
                                            var returnMainId=data[i].returnMainId;
                                            ...
                                            if(doModelList){
                                                ...
                                                for(var x=0;x<doModelList.length;x++){
                                                    ddo=ddo+"<a href=\"<%=basePath%>pages/front/member/"+doModelList[x].url+"&returnMainId="+returnMainId+"\">"+doModelList[x].name+"</a>&nbsp;"
                                                }
                                            }
                                            console.log(ddo)
                                            ...
                                            for(var j=0;j<goodsList.length;j++){
                                                $("#orderTable").append("<tr>\n" +
                                                    "                \t<td class=\"first-child\">\n" +
                                                    "                        <div class=\"t1\">\n" +
                                                    "                        \t<div class=\"lf od-pic\"><img src=\"<%=basePath%>/resources/upload/goodsPic/"+goodsList[j].pic+"\"></div>\n" +
                                                    "                            <div class=\"lf od-txt\">\n" +
                                                    "                            \t"+goodsList[j].goodsName+"</p>                    \n" +
                                                    "                                <p class=\"color02\">下单时间："+createTime+"</p>\n" +
                                                    "                            </div>\n" +
                                                    "                            <div class=\"clr\"></div>\n" +
                                                    "                        </div>\n" +
                                                    "                    </td>\n" +
                                                    "                    <td class=\"color04\"><div class=\"d2\">￥"+goodsList[j].price+"</div></td>\n" +
                                                    "                    <td><div class=\"t2\">x"+goodsList[j].count+"<div></td>\n" +
                                                    "                </tr>");
                                            }
                                        }
                                    },"json");
