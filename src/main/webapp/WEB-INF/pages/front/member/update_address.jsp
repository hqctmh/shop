...
<div class="row content">
    <div class="lf myslider">
        ...
        <c:if test="${user.userType==0}">
            ...
            <a class="${active=="my_index"?"actives":""}" href="<%=basePath%>pages/front/member/myAllOrder?type=myAllOrder"><img
        ...
        <div class="my_info my_address">
            <form action="<%=addAddressUrl%>" method="post">
                <div class="mg_top_1">
                    ...
                    <p class="lf"><input class="text_input" type="text" name="name"  placeholder="收货人" value="${address.name}"></p>
                    <div class="clr"></div>
                </div>
                <div class="mg_top_1">
                    <label class="lf">手机号码：</label>
                    <p class="lf"><input class="text_input" type="text" name="phone" placeholder="手机号码" value="${address.phone}"></p>
                    <div class="clr"></div>
                </div>
                <div class="mg_top_1">
                    ...
                    <p class="lf">
                        <select name="provinceId" id="selPro">
                            ...
                            <c:forEach items="${provinceList}" var="province">
                                <option value="${province.id}" ${province.id==address.provinceId?"selected":""}>${province.title}</option>
                            </c:forEach>
                        </select>
                        <select name="cityId" id="selCity">
                            <c:forEach items="${cityList}" var="city">
                                <option value="${city.id}" ${city.id==address.cityId?"selected":""}>${city.title}</option>
                            </c:forEach>
                        </select>
                        <br><input class="text_input" type="text" name="address" placeholder="详细收货地址" value="${address.address}">
                    </p>
                    <div class="clr"></div>
                </div>
